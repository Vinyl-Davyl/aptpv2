...
const LearnContent: React.FC<LearnContentProps> = ({ file }) => {
  ...
  const currentIndex = steps.findIndex((step) => step.link.includes(file.split(".")[0]));
  ...
  return (
    <ContentContainer>
      ...
      <NavigationButtons>
        <NavigationButton onClick={handlePrevious} disabled={currentIndex === 0} aria-label="Previous module">
          <LeftOutlined /> Previous
        </NavigationButton>
        <NavigationButton onClick={handleNext} disabled={currentIndex === steps.length - 1} aria-label="Next module">
          Next <RightOutlined />
        </NavigationButton>
      </NavigationButtons>
    </ContentContainer>
